...
public class productName extends baseClass {
	...
	public void purchaseApi(Map<String, String> cardData, Map<String, String> productnameData) throws Exception {
		...
		if (productName.isEmpty() || CardNumber.isEmpty()) {
			Reporter.log("Skipping test - Empty productName", true);
		...
		if (statusCode == 202) {
			...
			Thread.sleep(1000);
			s2sMethod(productName, ExpectedStatus, PSP, CardNumber, Expiry, CVV, PaymentMethod);
			
		} else if (statusCode == 400 || statusCode == 422) {
			Reporter.log("productName rejected by API: " + City, true);
			
			String actualOutcome = "FAIL";
			
			if (ExpectedStatus.equalsIgnoreCase("Fail")) {
				testResult = "PASS";
				comment = "productName rejected as expected";
			} else {
				testResult = "FAIL";
				comment = "productName rejected but expected to pass";
			}

			Reporter.log(comment, true);
			ExcelWriteUtility.writeResults2s("s2s_Result", productName, ExpectedStatus, actualOutcome, comment, purchaseId != null ? purchaseId : "N/A", PSP, PaymentMethod);
			driver.quit();
			return;
			
		} else {
			Reporter.log("Unexpected response for city: " + productName + " -> Status: " + statusCode, true);
			testResult = "FAIL";
			comment = "Unexpected API response (Status: " + statusCode + ")";
			ExcelWriteUtility.writeResults2s("s2s_Result", productName, ExpectedStatus, "UNKNOWN", comment, purchaseId != null ? purchaseId : "N/A", PSP, PaymentMethod);
			driver.quit();
			return;
		}
	}

	public void s2sMethod(String productName, String ExpectedStatus, String PSP, String CardNumber, String Expiry, String CVV, String PaymentMethod) throws Exception {
		...
		if (purchaseId == null || purchaseId.isEmpty()) {
			...
			ExcelWriteUtility.writeResults2s("s2s_Result", productName, ExpectedStatus, "FAIL", comment, "N/A", PSP, PaymentMethod);
			driver.quit();
			return;
		}
		...
		if (response == null || response.statusCode() != 202) {
			...
			Reporter.log(comment, true);
			ExcelWriteUtility.writeResults2s("s2s_Result", productName, ExpectedStatus, actualOutcome, comment, purchaseId, PSP, PaymentMethod);
			driver.quit();
			return;
		}
		...
		if (PSP.equalsIgnoreCase("payu")) {
			pay.payForPayu(productName, purchaseId, ExpectedStatus, PaymentMethod);
		}
